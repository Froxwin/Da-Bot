  ...
  async run(message, args) { // eslint-disable-line complexity
    const usEmbed = new MessageEmbed()
        ...
        .setDescription(oneLine`${message.author.tag}
                              Unable to send you the help DM.
                              You probably have DMs disabled.`)
        .setTimestamp();
    ...
    if (args.command && !showAll) {
      if (commands.length === 1) {
        let help = stripIndents`${oneLine`Command **${commands[0].name}**:__ ${commands[0].description} ${commands[0].guildOnly ? ' (Usable only in servers)' : ''} ${commands[0].nsfw ? ' (NSFW)' : ''}`} **Format:** ${message.anyUsage(`${commands[0].name}${commands[0].format ? ` ${commands[0].format}` : ''}`)}`;
        if (commands[0].aliases.length > 0) help += `\n**Aliases:** ${commands[0].aliases.join(', ')}`;
        help += `\n${oneLine`**Group:** ${commands[0].group.name} (\`${commands[0].groupID}:${commands[0].memberName}\`)`}`;
        ...
        return message.reply(`Unable to identify command. Use ${message.usage(null, message.channel.type === 'dm' ? null : undefined, message.channel.type === 'dm' ? null : undefined)} to view the list of all commands.`,
        );
      }
    } else {
      const messages = [];
      try {
        messages.push(await message.direct(stripIndents`${oneLine`To run a command in ${message.guild ? message.guild.name : 'any server'}, use ${Command.usage('command', message.guild ? message.guild.commandPrefix : null, this.client.user)}. For example, ${Command.usage('prefix', message.guild ? message.guild.commandPrefix : null, this.client.user)}.`} To run a command in this DM, simply use ${Command.usage('command', null, null)} with no prefix. Use ${this.usage('<command>', null, null)} to view detailed information about a specific command. Use ${this.usage('all', null, null)} to view a list of *all* commands, not just available ones. __**${showAll ? 'All commands' : `Available commands in ${message.guild || 'this DM'}`}**__ ${groups.filter((grp) => grp.commands.some((cmd) => !cmd.hidden && (showAll || cmd.isUsable(message)))).map((grp) => stripIndents` __${grp.name}__ ${grp.commands.filter((cmd) => !cmd.hidden && (showAll || cmd.isUsable(message))).map((cmd) => `**${cmd.name}:** ${cmd.description}${cmd.nsfw ? ' (NSFW)' : ''}`).join('\n')}`).join('\n\n')}`, {split: true}));
        if (message.channel.type !== 'dm') messages.push(await message.channel.send(sEmbed));
      } catch (err) {
        messages.push(await message.channel.send(usEmbed));
      }
